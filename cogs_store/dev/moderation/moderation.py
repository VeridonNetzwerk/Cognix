...
class Moderation(commands.Cog):
    ...
    async def warn(
        ...
        async with db_session() as s:
            ...
            warn_count = await s.scalar(
                select(func.count()).select_from(Warning_).where(
                    Warning_.server_id == guild_id,
                    Warning_.target_id == user.id,
                )
            ) or 0
            cfg = await s.scalar(
                select(ServerConfig).where(ServerConfig.server_id == guild_id)
            )
            escalation = (cfg.extras if cfg else {}).get("warn_escalation", {})
            threshold = escalation.get("threshold")
            mute_secs = escalation.get("mute_duration_seconds", 3600)
        ...
        if threshold and warn_count >= threshold:
            try:
                until = _now() + timedelta(seconds=min(mute_secs, 28 * 86400))
                await user.timeout(until, reason=f"Auto-mute: {warn_count} warnings")
                await _record(
                    guild_id=guild_id,
                    action=ModerationActionType.MUTE,
                    moderator_id=interaction.user.id,
                ...
                await interaction.response.send_message(
                    embed=ok_embed(
                        "User warned + auto-muted",
                        f"{user.mention} reached {warn_count} warnings and was muted for {humanize_seconds(mute_secs)}.",
                    ),
                    ephemeral=True,
                )
                return
            except discord.HTTPException:
                log.warning("warn_escalation_mute_failed", user_id=user.id, exc_info=True)

        await interaction.response.send_message(embed=ok_embed("User warned"), ephemeral=True)

    @app_commands.command(name="warn-config", description="Configure auto-mute threshold for warnings")
    @app_commands.describe(
        threshold="Number of warnings before auto-mute (0 = disabled)",
        mute_duration="Mute duration, e.g. 10m, 1h, 1d (default 1h)",
    )
    @app_commands.checks.has_permissions(moderate_members=True)
    async def warn_config(
        self,
        interaction: discord.Interaction,
        threshold: app_commands.Range[int, 0, 50],
        mute_duration: str = "1h",
    ) -> None:
        guild_id = interaction.guild_id
        if guild_id is None:
            await interaction.response.send_message("Guild only", ephemeral=True)
            return
        secs = parse_duration(mute_duration) or 3600
        async with db_session() as s:
            cfg = await s.scalar(
                select(ServerConfig).where(ServerConfig.server_id == guild_id)
            )
            if cfg is None:
                cfg = ServerConfig(server_id=guild_id)
                s.add(cfg)
            extras = dict(cfg.extras or {})
            extras["warn_escalation"] = {
                "threshold": threshold,
                "mute_duration_seconds": secs,
            }
            cfg.extras = extras
        if threshold == 0:
            await interaction.response.send_message(
                embed=ok_embed("Warn escalation disabled"), ephemeral=True
            )
        else:
            await interaction.response.send_message(
                embed=ok_embed(
                    "Warn escalation configured",
                    f"Auto-mute after **{threshold}** warnings for {humanize_seconds(secs)}.",
                ),
                ephemeral=True,
            )
