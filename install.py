...
def create_admin(python: str) -> None:
    ...
    if answer and answer != "y":
        ...
        warn(f"  {python} -m scripts.create_admin <username> <email> <password>")
        return
    ...
    cmd = [python, "-m", "scripts.create_admin", username]
    if email:
        cmd.append(email)
    cmd.append(password)
    run(cmd)
...
def print_next_steps(python: str) -> None:
  ...
  {BOLD}Useful commands:{RESET}
     ...
     Create admin:  {python} -m scripts.create_admin <username> <email> <password>
     Health check:  {python} -m scripts.healthcheck
